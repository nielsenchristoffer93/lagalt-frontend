...
const AdminView = (props) => {
    ...
    const fetchProjectStatusWithUrl = async (url) => {
        let statusObj = await fetchProjectStatus(url).then(response => response.json())
    ...
    const handleUpdateProject = async () => {
        ...
        const updatedProject = await updateProject(formData, selectedProject.id).then(response => response.json());
    ...
    return (
        ...
        <br/>
        <Button onClick={() => handleUpdateProject()} variant="success">Update project</Button>
        {postSuccessful ? <Alert variant="success" style={{float:"right", padding:"6px"}}>
                    Update successful!
                </Alert> : null}
      </Modal.Body>
    )
